...
write.table(weather, "/Users/rozenn/Library/CloudStorage/GoogleDrive-rozennpineau@uchicago.edu/My Drive/Work/9.Science/1.DroughtProject/1.analyses/data/5.herbarium/climate_datasets/temperature/climate_NOAA_2000_2005.txt", sep = "\t",
              col.names = F, row.names = F, quote = F)


station_number <- c()
years <- unique(meta$Year)[unique(meta$Year) >= 2000 & unique(meta$Year) < 2005]
      ...
      for (station in idx_stations_50k[order(dist[idx_stations_50k])]) {
          ...
          #go to next one #else if { #if last station has been reached and still no temperature data, take the closest one (I already have that dataset)
            ...
            if (dim(weather)[1] > 1) {
                ...
                #build file
                 write.table(weather, "/Users/rozenn/Library/CloudStorage/GoogleDrive-rozennpineau@uchicago.edu/My Drive/Work/9.Science/1.DroughtProject/1.analyses/data/5.herbarium/climate_datasets/temperature/climate_NOAA_2000_2005.txt",
